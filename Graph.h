...
vector<Vertex<T>*> Graph<T>::calculateKruskal() {
    ...
    for(Vertex<T>* i : this->vertexSet)
    {
        i->disjSet.push_back(i);
        for(Edge<T> &j : i->adj){
            q.push(j);
        }
    }
    ...
    while (edgesAccepted < this->vertexSet.size() - 1){
        Edge<T> e = q.top();
        q.pop();
        vector<Vertex<T>*> uset = e.orig->disjSet;
        vector<Vertex<T>*> vset = e.dest->disjSet;
        if(!setEquals(uset, vset)){
            edgesAccepted++;
            uset.insert(uset.end(), vset.begin(), vset.end());
            e.orig->disjSet = uset;
            e.dest->disjSet = uset;
            e.orig->path = e.dest;
        }
